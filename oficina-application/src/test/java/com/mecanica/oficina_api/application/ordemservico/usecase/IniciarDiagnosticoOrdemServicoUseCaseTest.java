...
class IniciarDiagnosticoOrdemServicoUseCaseTest {
    ...
    @InjectMocks
    private IniciarDiagnosticoOrdemServicoUseCase useCase;

    @Test
    void deveIniciarDiagnostico_quandoOsRecebida() {
        ...
        assertThat(resultado.getStatus()).isEqualTo(OrdemServicoStatus.EM_DIAGNOSTICO);
        verify(ordemServicoGateway).atualizar(os);
    }

    @Test
    void deveLancarExcecao_quandoOsNaoEstaRecebida() {
        OrdemServico os = osSemOrcamento(OrdemServicoStatus.EM_EXECUCAO);
        when(ordemServicoGateway.encontrarOuLancar(OS_ID)).thenReturn(os);

        assertThatThrownBy(() -> useCase.executar(OS_ID))
            .isInstanceOf(IllegalStateException.class)
            .hasMessageContaining("OS deve estar RECEBIDA");
